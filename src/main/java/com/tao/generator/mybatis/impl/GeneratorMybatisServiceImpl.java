package com.tao.generator.mybatis.impl;
...
import com.tao.generator.mybatis.enums.FieldTypeEnum;
import com.tao.generator.mybatis.template.GeneratorGetDto;
import com.tao.generator.mybatis.template.GeneratorModel;
import com.tao.generator.mybatis.template.GeneratorReqDto;
import com.tao.generator.mybatis.engine.FreemarkerTemplateEngine;
import com.tao.generator.mybatis.enums.QueryConditionEnum;
...
public class GeneratorMybatisServiceImpl implements GeneratorMybatisService {
...
//        generatorCodeXmlCustomMapper(req, generatorModel);
    ...
    public String generatorCodeList(GeneratorListReq req) throws Exception {
        ...
        FreemarkerTemplateEngine.writer(objectMap, "\\mybatis\\sql\\list\\service\\dto\\Req.java.ftl", outputFullPathName);

        //生成Rsp.java
        outputFullPathName = req.getOutputPath()
                + "\\java\\" + servicePackageName.replace(".", "\\")
                + "\\dto\\" + req.getDtoName() + "Rsp.java";
        FreemarkerTemplateEngine.writer(objectMap, "\\mybatis\\sql\\list\\service\\dto\\RspDTO.java.ftl", outputFullPathName);
        ...
        String serviceFile = req.getOutputPath()
                ...
                + ".java";
        String serviceContent = FreemarkerTemplateEngine.generateStringFromTemplate(objectMap,
                "\\mybatis\\sql\\list\\service\\Service.java.ftl");
        ClassUtils.addMethod(serviceFile, serviceContent);
        ...
        String serviceImplFile = req.getOutputPath()
                ...
                + "\\impl\\" + serviceClassJavaName + "Impl.java";
        String serviceImplContent = FreemarkerTemplateEngine.generateStringFromTemplate(objectMap,
                "\\mybatis\\sql\\list\\service\\impl\\ServiceImpl.java.ftl");
        ClassUtils.addMethod(serviceImplFile, serviceImplContent);
        ...
        String controllerFile = req.getOutputPath()
                ...
                + ".java";
        String controllerContent = FreemarkerTemplateEngine.generateStringFromTemplate(objectMap,
                "\\mybatis\\sql\\list\\controller\\Controller.java.ftl");
        ClassUtils.addMethod(controllerFile, controllerContent);
        ...
        String mapperJavaFile = req.getOutputPath()
                ...
                + ".java";
        String mapperJavaContent = FreemarkerTemplateEngine.generateStringFromTemplate(objectMap,
                "\\mybatis\\sql\\list\\mapper\\custom\\CustomMapper.java.ftl");
        ClassUtils.addMethod(mapperJavaFile, mapperJavaContent);
        ...
        String mapperXmlFile = req.getOutputPath()
                ...
                + mapperClassJavaName + ".xml";
        String mapperXmlContent = FreemarkerTemplateEngine.generateStringFromTemplate(objectMap,
                "\\mybatis\\sql\\list\\mybatis\\custom\\customMapper.xml.ftl");
        XmlUtils.addMethod(mapperXmlFile, mapperXmlContent);
        return null;
    }
    ...
    private void generatorCodeJavaController(GeneratorReq req, GeneratorModel generatorModel) throws Exception {
        ...
        String outputFullPathName = req.getOutputPath()
                ...
                + "Controller.java";
        FreemarkerTemplateEngine.writer(objectMap, "\\mybatis\\java\\controller\\Controller.java.ftl", outputFullPathName);
    }

    private void generatorCodeJavaServiceImpl(GeneratorReq req, GeneratorModel generatorModel) throws Exception {
        String[] files = {"ServiceImpl.java"};
        Map<String, Object> objectMap = ObjectUtils.convertPojoToHashMap(generatorModel);
        for (String dtoFile : files) {
            String outputFullPathName = req.getOutputPath()
                    + "\\java\\" + req.getPackageName().replace(".", "\\")
                    + "\\service\\impl\\" + generatorModel.getTable().getNameUpperCamelCase()
                    + dtoFile;
            FreemarkerTemplateEngine.writer(objectMap, "\\mybatis\\java\\service\\impl\\" + dtoFile + ".ftl", outputFullPathName);
        }
    }

    private void generatorCodeJavaService(GeneratorReq req, GeneratorModel generatorModel) throws Exception {
        String[] files = {"Service.java"};
        Map<String, Object> objectMap = ObjectUtils.convertPojoToHashMap(generatorModel);
        for (String dtoFile : files) {
            String outputFullPathName = req.getOutputPath()
                    + "\\java\\" + req.getPackageName().replace(".", "\\")
                    + "\\service\\" + generatorModel.getTable().getNameUpperCamelCase()
                    + dtoFile;
            FreemarkerTemplateEngine.writer(objectMap, "\\mybatis\\java\\service\\" + dtoFile + ".ftl", outputFullPathName);
        }
    }

    private void generatorCodeJavaDto(GeneratorReq req, GeneratorModel generatorModel) throws Exception {
        String[] dtoFiles = {"InsertReq.java", "UpdateReq.java","ListReq.java"};
        Map<String, Object> objectMap = ObjectUtils.convertPojoToHashMap(generatorModel);
        for (String dtoFile : dtoFiles) {
            String outputFullPathName = req.getOutputPath()
                    + "\\java\\" + req.getPackageName().replace(".", "\\")
                    + "\\domain\\dto\\" + generatorModel.getTable().getNameUpperCamelCase() + dtoFile;
            FreemarkerTemplateEngine.writer(objectMap, "\\mybatis\\java\\domain\\dto\\" + dtoFile + ".ftl", outputFullPathName);
        }
        String[] voFiles = {"Rsp.java","ListRsp.java"};
        for (String voFile : voFiles) {
            String outputFullPathName = req.getOutputPath()
                    + "\\java\\" + req.getPackageName().replace(".", "\\")
                    + "\\domain\\vo\\"+ generatorModel.getTable().getNameUpperCamelCase() + voFile;
            FreemarkerTemplateEngine.writer(objectMap, "\\mybatis\\java\\domain\\vo\\" + voFile + ".ftl", outputFullPathName);
        }
    }
    ...
    private void generatorCodeJavaCustomMapper(GeneratorReq req, GeneratorModel generatorModel) throws Exception {
        ...
        FreemarkerTemplateEngine.writer(objectMap, "\\mybatis\\java\\mapper\\Mapper.java.ftl", outputFullPathName);
    }
    ...
    private void generatorCodeXmlMapper(GeneratorReq req, GeneratorModel generatorModel) throws Exception {
        ...
        String outputFullPathName = req.getOutputPath()
                + "\\java\\" + req.getPackageName().replace(".", "\\")
                + "\\mapper\\xml\\" + generatorModel.getMapperClassName() + ".xml";
        FreemarkerTemplateEngine.writer(objectMap, "\\mybatis\\java\\mapper\\xml\\Mapper.xml.ftl", outputFullPathName);
    }
    ...
    private void transformTable(GeneratorReq req, Table table) {
        ...
        table.setNameUpperCamelCase(StringUtils.upperCamelCase(table.getName().replace(req.getTablePrefix(),"")));
        table.setNameLowerCamelCase(StringUtils.lowerCamelCase(table.getName().replace(req.getTablePrefix(),"")));
...
//            log.info("field:{}", field);
    ...
    public void generatorCodeJavaEntity(GeneratorReq req, GeneratorModel generatorModel) throws Exception {
        ...
        String outputFullPathName = req.getOutputPath()
                + "\\java\\" + req.getPackageName().replace(".", "\\")
                + "\\domain\\entity\\" + generatorModel.getEntityClassName() + ".java";
        FreemarkerTemplateEngine.writer(objectMap, "\\mybatis\\java\\domain\\entity\\Entity.java.ftl", outputFullPathName);
    }
}